...
#pragma pack(push, 1)
    typedef struct Data
    {
        ...
        unsigned int cbName;
        unsigned int nCatalogID;
        unsigned int nEphemerisType;
        unsigned int nLaunchYear;
        long long tEpoch;
        double fMeanMotion;
        double fMeanMotionDerivativeDt;
        double fMeanMotionDerivativeDdt;
        double fMeanAnomaly;
        double fInclination;
        double fEccentricity;
        double fArgOfPerigee;
        double fRAOfAscendingNode;
        double fBStarDragTerm;
    } Data;
#pragma pack(pop)

public:
   cTle();
   cTle(const string& strName, const string& strLine1, const string& strLine2);
   cTle(const string& strName, const Data& data);
   cTle(const cTle &tle);
   ~cTle();
   ...
   double GetEpoch() const
   {
       ...
       return cJulian(epochYear, GetField(FLD_EPOCHDAY)).Date();
   }
   ...
   operator tle_t() const
   {
       tle_t  tle;

       tle.epoch = GetEpoch();
       tle.eo = GetField(cTle::FLD_E);
       tle.bstar = GetField(cTle::FLD_BSTAR);
       tle.xmo = GetField(cTle::FLD_M, cTle::U_RAD);
       tle.xincl = GetField(cTle::FLD_I, cTle::U_RAD);
       tle.xnodeo = GetField(cTle::FLD_RAAN, cTle::U_RAD);
       tle.omegao = GetField(cTle::FLD_ARGPER, cTle::U_RAD);
       tle.xno = (TWOPI * GetField(cTle::FLD_MMOTION)) / MIN_PER_DAY;
       tle.xndt2o = (TWOPI * GetField(cTle::FLD_MMOTIONDT)) / (MIN_PER_DAY * MIN_PER_DAY);
       tle.xndd6o = (TWOPI * GetField(cTle::FLD_MMOTIONDT2)) / (MIN_PER_DAY * MIN_PER_DAY * MIN_PER_DAY);
       tle.ephemeris_type = select_ephemeris(&tle);
       return tle;
   }
