...
CTimeKey::CTimeKey(INT nYear, INT nDayofYear, INT nHour, INT nMinute, INT nSecond) : CTime()
{
	...
	tTime.tm_wday = 0;
	tTime.tm_isdst = -1;
	SetTime(((tTimeKey = _mktime64(&tTime) + ((time_t)nDayofYear - 1)*SECONDSPERDAY) < MIN_TIMEKEY || tTimeKey > MAX_TIMEKEY) ? min(max(tTimeKey, MIN_TIMEKEY), MAX_TIMEKEY) : tTimeKey);
}
		...
		for (nTimeSubkey = 0, nTimeSubkeys = (INT)szTimeSubkeys.GetSize(); nTimeSubkey < nTimeSubkeys; nTimeSubkey++)
		{
			if (szSubFormats.GetAt(nTimeSubkey) == STRING(nSubFormatID))
			{
				...
				if (nSubFormatID == IDS_TIMEKEY_SUBFORMAT_WEEKOFYEAR)
				{
					...
					CTimeKey  tTimeConversion(CTimeKey(sTimeKeyInfo).GetTime() + (TIMEKEY)(((TIMETAG)nWeekOfYear - (TIMETAG)_ttoi(CTimeKey(sTimeKeyInfo).Format(nSubFormatID)))*SECONDSPERWEEK));
				...
				if (nSubFormatID == IDS_TIMEKEY_SUBFORMAT_DAYOFWEEK)
				{
					...
					CTimeKey  tTimeConversion(CTimeKey(sTimeKeyInfo).GetTime() + (TIMEKEY)(((TIMETAG)nDayOfWeek - (TIMETAG)CTimeKey(sTimeKeyInfo).GetDayOfWeek())*SECONDSPERDAY));
				...
				if (nSubFormatID == IDS_TIMEKEY_SUBFORMAT_WEEKDAYNAME || nSubFormatID == IDS_TIMEKEY_SUBFORMAT_WEEKDAYMNEMONIC)
				{
					...
					CTimeKey  tTimeConversion(CTimeKey(sTimeKeyInfo).GetTime() + (TIMEKEY)(((TIMETAG)(nDayOfWeek%DAYSPERWEEK) - (TIMETAG)CTimeKey(sTimeKeyInfo).GetDayOfWeek() + 1)*SECONDSPERDAY));
					sTimeKeyInfo.wDayOfWeek = (WORD)tTimeConversion.GetDayOfWeek() - 1;
					sTimeKeyInfo.wMonth = (WORD)tTimeConversion.GetMonth();
					sTimeKeyInfo.wDay = (WORD)tTimeConversion.GetDay();
					break;
...
{
	return m_nMicroSeconds % 1000000;
}
...
CString CTimeTag::FormatTime(LPCTSTR pszFormat, BOOL bUTC) CONST
{
	...
	if (bUTC)
	{
		for (szFormat[0] = pszFormat; GetTime() < 0; )
		{
			...
			{
				szFormat[1] = cStringTools.ConvertIntToString(-1).GetAt(0) + cStringTools.ConvertIntToPaddedString(GetMicroSeconds(), 6);
		...
		if ((nPos = szFormat[0].Find(STRING(IDS_TIMEKEY_SUBFORMAT_MICROSECOND))) >= 0)
		{
			...
			szFormat[0] = szFormat[0].Left(nPos) + cStringTools.ConvertIntToPaddedString(abs(GetMicroSeconds()), 6) + szFormat[0].Mid(nPos);
		}
...
TIMETAG CTimeTag::ParseTime(LPCTSTR pszTimeTag, LPCTSTR pszFormat, BOOL bUTC) CONST
{
	...
	for (nSubFormatID = (szTimeSubkeys.GetSize() == szSubFormats.GetSize() && nSubFormatID >= 0) ? MINTIMETAGSUBFORMAT : -1; nSubFormatID >= 0 && nSubFormatID <= MAXTIMETAGSUBFORMAT; nSubFormatID++)
	{
		for (nTimeSubkey = 0, nTimeSubkeys = (INT)szTimeSubkeys.GetSize(); nTimeSubkey < nTimeSubkeys; nTimeSubkey++)
		{
			if (szSubFormats.GetAt(nTimeSubkey) == STRING(nSubFormatID))
			{
				if (nSubFormatID == IDS_TIMEKEY_SUBFORMAT_MILLISECOND)
				{
					if ((nMilliSeconds = _ttoi(szTimeSubkeys.GetAt(nTimeSubkey))) >= LOWORD(EnumSubFormatLimits(nSubFormatID, TRUE)) && nMilliSeconds <= HIWORD(EnumSubFormatLimits(nSubFormatID, TRUE)))
					{
						nMicroSeconds = 0;
						break;
					}
					if (nMilliSeconds >= -(INT)HIWORD(EnumSubFormatLimits(nSubFormatID, TRUE)) && nMilliSeconds <= -(INT)LOWORD(EnumSubFormatLimits(nSubFormatID, TRUE)))
					{
						nMicroSeconds = 0;
						break;
					}
	...
	return(((tTime = (nSubFormatID >= 0) ? (1000000 * (TIMETAG)CTimeKey::ParseTime(pszTimeTag, pszFormat, bUTC, bNegative)) : -1) != (TIMEKEY)-1 || bNegative) ? (((tTime >= 0 && ((nMilliSeconds >= 0 && nMicroSeconds >= 0) || (nMilliSeconds < 0 && nMicroSeconds >= 0 && !bNegative) || (nMilliSeconds >= 0 && nMicroSeconds < 0 && !bNegative))) || ((tTime < 0 || bNegative) && nMilliSeconds >= 0 && nMicroSeconds >= 0)) ? ((tTime >= 0 && nMilliSeconds >= 0 && nMicroSeconds >= 0 && !bNegative) ? (tTime + 1000 * (TIMETAG)nMilliSeconds + (TIMETAG)nMicroSeconds) : (-abs(tTime) - 1000 * (TIMETAG)abs(nMilliSeconds) - (TIMETAG)abs(nMicroSeconds))) : -1) : -1);
}
